...
-- Below, changing the format is OK, e.g., "op x r = ...".
    z = (\b -> b) -- this is the base case i.e. when we have say empty list then we shud return the b or the acc.
    op = (\x r b -> r (f b x))  -- we change the x and r because we are making a foldl.
